...
metadata {
	definition (name: "egauge System", namespace: "gouldner", author: "Ronald Gouldner") {
    ...
    fingerprint deviceId: "RRGegauge"
	}
...
def energyRefresh() {  
  ...
  def cmd = "http://egauge15753.egaug.es/cgi-bin/egauge?noteam";
        ...
        if (resp.data) {
            ...
            resp.data.meter.each { 
                ...
                if (it.name().equals("meter") && it.@title.equals("Solar")) {
                    log.debug "Found Power"
                    currentPower = it.power
            ...
            */
             ...
             // String.format("%5.2f", energyToday)
            delayBetween([sendEvent(name: 'power', value: (currentPower))
	                     ])
